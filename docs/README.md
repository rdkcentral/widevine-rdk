...
Runtime storage for DRM artifacts is handled in two ways: the keybox path is passed to the CDM via an environment variable (`WIDEVINE_KEYBOX_PATH`), and certificate data can be pre-loaded into the in-memory `HostImplementation` store before the CDM is initialized. The `HostImplementation` uses an STL `std::map<std::string, std::string>` as its backing store, so CDM storage reads/writes are process-lifetime unless the caller re-seeds the store on startup.
